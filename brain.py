    ...
    def __init__(self, inputs, clone=False):
        ...
        if not clone:
            ...
            for i in range(0, self.inputs):
                self.nodes.append(neuronne.Node(i))
                self.nodes[i].layer = 0
            # Create bias node
            self.nodes.append(neuronne.Node(3))
            self.nodes[3].layer = 0
            # Create output node
            self.nodes.append(neuronne.Node(4))
            self.nodes[4].layer = 1

            # Create connections
            for i in range(0, 4):
                self.connections.append(synapse.Connection(self.nodes[i],
                                                              self.nodes[4],
                                                              random.uniform(-1, 1)))
